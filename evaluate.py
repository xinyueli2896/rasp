...
@torch.no_grad()
def generate_with_log_probs(model, start_tokens, n_new):
    """Greedy generation plus next-token distributions for each generated position."""
    model.eval()
    tokens = start_tokens.clone()
    log_probs = []
    distributions = []
    max_len = getattr(model, "max_seq_len", None)
    if max_len is None and hasattr(model, "_ar_base"):
        max_len = model._ar_base.max_seq_len

    for _ in range(n_new):
        ctx = tokens if max_len is None else tokens[:, -max_len:]
        logits = model(ctx)
        next_log_probs = F.log_softmax(logits[:, -1, :], dim=-1)
        next_probs = next_log_probs.exp()
        next_tok = next_log_probs.argmax(dim=-1, keepdim=True)
        chosen_log_prob = next_log_probs.gather(1, next_tok).squeeze(1)
        log_probs.append(float(chosen_log_prob.item()))
        distributions.append(next_probs.squeeze(0).cpu().tolist())
        tokens = torch.cat([tokens, next_tok], dim=1)

    return tokens, log_probs, distributions
...
def run_evaluation(args):
    ...
    print()
    qual_prompt_len = 1
    print("Qualitative generation examples (prompt length = 1)")
    print("-" * 72)
    show_len = 9
    qual_models = [
        ("Pretrain", pretrain_model),
    ...
    for cat_label, x in (
        ...
        prompt   = torch.tensor([seq[:qual_prompt_len]], dtype=torch.long, device=device)
        expected = seq[qual_prompt_len: qual_prompt_len + show_len]

        def mark(gen): return "✓" if gen == expected else "✗"
        print(f"  [{cat_label}] x={x}  prompt={seq[:qual_prompt_len]}")
        print(f"    Expected : {expected}")
        for mdl_label, mdl in qual_models:
            gen_tokens, gen_log_probs, gen_distributions = generate_with_log_probs(mdl, prompt, show_len)
            gen = gen_tokens[0, qual_prompt_len:].cpu().tolist()
            print(f"    {mdl_label:<14}: {gen}  {mark(gen)}")
            should_print_wrong_details = cat_label.strip() == "Neither" and mdl_label.startswith("skip=")
            if not should_print_wrong_details:
                continue
            wrong_positions = [
                (pos, tok, exp, log_p, dist)
                for pos, (tok, exp, log_p, dist) in enumerate(
                    zip(gen, expected, gen_log_probs, gen_distributions), start=1
                )
                if tok != exp
            ]
            if wrong_positions:
                wrong_str = ", ".join(
                    f"pos {pos}: pred={tok}, exp={exp}, log p={log_p:.4f}, "
                    f"p(target)={dist[exp]:.4f}, "
                    f"top3={[(i, round(p, 4)) for i, p in sorted(enumerate(dist), key=lambda x: x[1], reverse=True)[:3]]}"
                    for pos, tok, exp, log_p, dist in wrong_positions
                )
                print(f"    {'':14}  wrong preds: {wrong_str}")
            else:
                print(f"    {'':14}  wrong preds: none")
        print()
