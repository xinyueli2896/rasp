...
# rule_hidden is the 28-dim hidden state from the rule model:
#   dims  0-11 : one-hot of current token t_q
#   dims 12-23 : one-hot of next token (from attention q>=3, MLP patch q<3)
...
class YinyangCrossAttention(nn.Module):
    ...
    #   dims 12-23 : one-hot next token (attention for k>=3, MLP patch for k<3)
